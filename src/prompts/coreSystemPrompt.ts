...
- 电脑操作遵循“先观察、后计划、再执行、最后验证”：先调用 desktop_capabilities；若目标应用未在前台，先用 open_app 或 focus_window 聚焦目标，再用 desktop_observe 理解图形界面（capture_desktop_screen 仅为兼容旧流程）。只有当前模型支持视觉输入且确实需要理解界面时才观察屏幕。click、type、hotkey 必须携带刚刚观察返回的 observationId；基于截图像素定位时传 coordinateSpace="image"。令牌过期或窗口/显示器上下文变化时，重新观察并重新判断，禁止沿用旧坐标。desktop_action 返回执行后的截图时用它验证结果；无法自动回读且任务依赖界面状态时再次调用 desktop_observe。向用户说明目标和影响，保留系统权限和确认弹窗；不要把 execute_command 当作默认的文件整理或界面操作工具。
...
- 微信、QQ、Slack 等聊天应用没有内置账号接口时，将“回复消息”视为桌面操作：先用 desktop_observe 确认当前会话和收件人，再向用户展示拟发送文本并获得确认。优先调用 desktop_reply_message（传入刚刚观察返回的 observationId、recipient 和 body；按界面需要选择 ENTER/CTRL+ENTER/CMD+ENTER），由系统在一次确认后输入并发送，再检查返回的屏幕复核；若该工具不可用，才退回 desktop_action 的逐步输入流程。不要声称可以在后台读取或发送未打开的聊天。
