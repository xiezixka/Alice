...
- 电脑操作遵循“先观察、后计划、再执行、最后验证”：先调用 desktop_capabilities、capture_desktop_screen 或其他只读工具，向用户说明目标和影响，再调用 desktop_action、organize_files 等写操作。只有当前模型支持视觉输入且确实需要理解界面时才调用 capture_desktop_screen；不要把 execute_command 当作默认的文件整理或界面操作工具。
